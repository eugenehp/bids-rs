...
pub fn read_blood_tsv(path: &std::path::Path) -> Result<Vec<BloodSample>> {
    ...
    Ok(rows
        ...
        .map(|r| BloodSample {
            ...
            whole_blood_radioactivity: r
                .get("whole_blood_radioactivity")
                .and_then(|v| v.parse().ok()),
            plasma_radioactivity: r.get("plasma_radioactivity").and_then(|v| v.parse().ok()),
            metabolite_parent_fraction: r
                .get("metabolite_parent_fraction")
                .and_then(|v| v.parse().ok()),
        })
        .collect())
}

pub struct PetLayout<'a> {
    layout: &'a BidsLayout,
}
impl<'a> PetLayout<'a> {
    ...
    pub fn get_pet_files(&self) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("pet").collect()
    }
    pub fn get_pet_files_for_subject(&self, s: &str) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("pet").subject(s).collect()
    }
    pub fn get_pet_files_for_task(&self, t: &str) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("pet").task(t).collect()
    }
    pub fn get_blood(&self, f: &BidsFile) -> Result<Option<Vec<BloodSample>>> {
        ...
        if p.exists() {
            return Ok(Some(read_blood_tsv(&p)?));
        }
        ...
        for entry in std::fs::read_dir(&f.dirname)
            .into_iter()
            .flatten()
            .flatten()
        {
            ...
            if name.contains("blood")
                && name.ends_with(".tsv")
                && name.starts_with(&base[..base.len().min(10)])
            {
    ...
    pub fn get_metadata(&self, f: &BidsFile) -> Result<Option<PetMetadata>> {
    ...
    pub fn get_pet_subjects(&self) -> Result<Vec<String>> {
        self.layout.get().suffix("pet").return_unique("subject")
    }
    pub fn get_pet_tasks(&self) -> Result<Vec<String>> {
        self.layout.get().suffix("pet").return_unique("task")
    }
