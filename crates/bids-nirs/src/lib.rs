...
pub struct NirsMetadata {
    #[serde(default)] pub sampling_frequency: Option<f64>,
    #[serde(rename = "NIRSChannelCount", default)] pub nirs_channel_count: Option<u32>,
    #[serde(default)] pub task_name: Option<String>,
    #[serde(default)] pub manufacturer: Option<String>,
    #[serde(rename = "NIRSSourceOptodeCount", default)] pub source_optode_count: Option<u32>,
    #[serde(rename = "NIRSDetectorOptodeCount", default)] pub detector_optode_count: Option<u32>,
}
impl NirsMetadata { pub fn from_metadata(md: &BidsMetadata) -> Option<Self> { md.deserialize_as() } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Optode { pub name: String, pub optode_type: String, pub x: Option<f64>, pub y: Option<f64>, pub z: Option<f64> }
impl Optode {
...
pub struct NirsLayout<'a> { layout: &'a BidsLayout }
impl<'a> NirsLayout<'a> {
    pub fn new(layout: &'a BidsLayout) -> Self { Self { layout } }
    pub fn get_nirs_files(&self) -> Result<Vec<BidsFile>> { self.layout.get().suffix("nirs").collect() }
    pub fn get_nirs_files_for_subject(&self, s: &str) -> Result<Vec<BidsFile>> { self.layout.get().suffix("nirs").subject(s).collect() }
    pub fn get_nirs_files_for_task(&self, t: &str) -> Result<Vec<BidsFile>> { self.layout.get().suffix("nirs").task(t).collect() }
    pub fn get_channels(&self, f: &BidsFile) -> Result<Option<Vec<bids_eeg::Channel>>> {
        let p = f.companion("channels", "tsv");
        if p.exists() { Ok(Some(bids_eeg::read_channels_tsv(&p)?)) } else { Ok(None) }
    }
    pub fn get_optodes(&self, f: &BidsFile) -> Result<Option<Vec<Optode>>> {
        let p = f.companion("optodes", "tsv");
        if p.exists() { Ok(Some(read_optodes_tsv(&p)?)) } else { Ok(None) }
    }
    pub fn get_events(&self, f: &BidsFile) -> Result<Option<Vec<bids_eeg::EegEvent>>> {
        let p = f.companion("events", "tsv");
        if p.exists() { Ok(Some(bids_eeg::read_events_tsv(&p)?)) } else { Ok(None) }
    }
    pub fn get_coordsystem(&self, f: &BidsFile) -> Result<Option<bids_eeg::CoordinateSystem>> {
        let p = f.companion("coordsystem", "json");
        if p.exists() { Ok(Some(bids_eeg::CoordinateSystem::from_file(&p)?)) } else { Ok(None) }
    }
    pub fn get_metadata(&self, f: &BidsFile) -> Result<Option<NirsMetadata>> { Ok(self.layout.get_metadata(&f.path)?.deserialize_as()) }
    ...
    pub fn get_all_channels_files(&self) -> Result<Vec<BidsFile>> { self.layout.get().suffix("channels").datatype("nirs").extension("tsv").collect() }
    pub fn get_all_optodes_files(&self) -> Result<Vec<BidsFile>> { self.layout.get().suffix("optodes").datatype("nirs").extension("tsv").collect() }
    pub fn get_nirs_subjects(&self) -> Result<Vec<String>> { self.layout.get().suffix("nirs").return_unique("subject") }
    pub fn get_nirs_tasks(&self) -> Result<Vec<String>> { self.layout.get().suffix("nirs").return_unique("task") }
    pub fn summary(&self) -> Result<NirsSummary> {
        ...
        Ok(NirsSummary { n_subjects: subjects.len(), n_recordings: files.len(), subjects, tasks, sampling_frequency: sf, channel_count: n_ch.map(|c| c as usize) })
    }
}

#[derive(Debug)]
pub struct NirsSummary { pub n_subjects: usize, pub n_recordings: usize, pub subjects: Vec<String>, pub tasks: Vec<String>, pub sampling_frequency: Option<f64>, pub channel_count: Option<usize> }
impl std::fmt::Display for NirsSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "NIRS Summary: {} subjects, {} recordings, tasks: {:?}", self.n_subjects, self.n_recordings, self.tasks)?;
        if let Some(sf) = self.sampling_frequency { writeln!(f, "  Sampling: {sf} Hz")?; }
        if let Some(ch) = self.channel_count { writeln!(f, "  Channels: {ch}")?; }
        Ok(())
    }
}
