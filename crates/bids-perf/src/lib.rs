...
pub struct PerfMetadata {
    #[serde(default)] pub arterial_spin_labeling_type: Option<String>,
    #[serde(default)] pub post_labeling_delay: Option<serde_json::Value>,
    #[serde(default)] pub background_suppression: Option<bool>,
    #[serde(default)] pub m0_type: Option<String>,
    #[serde(default)] pub repetition_time_preparation: Option<f64>,
    #[serde(default)] pub echo_time: Option<f64>,
    #[serde(default)] pub flip_angle: Option<f64>,
    #[serde(default)] pub labeling_duration: Option<f64>,
    #[serde(default)] pub total_acquired_pairs: Option<u32>,
    #[serde(default)] pub vascular_crushing: Option<bool>,
...
pub struct PerfLayout<'a> { layout: &'a BidsLayout }
impl<'a> PerfLayout<'a> {
    pub fn new(layout: &'a BidsLayout) -> Self { Self { layout } }
    pub fn get_asl_files(&self) -> Result<Vec<BidsFile>> { self.layout.get().suffix("asl").collect() }
    pub fn get_asl_files_for_subject(&self, s: &str) -> Result<Vec<BidsFile>> { self.layout.get().suffix("asl").subject(s).collect() }
    pub fn get_m0scan_files(&self) -> Result<Vec<BidsFile>> { self.layout.get().suffix("m0scan").collect() }
    pub fn get_aslcontext(&self, f: &BidsFile) -> Result<Option<Vec<String>>> {
        ...
        Ok(Some(rows.iter().filter_map(|r| r.get("volume_type").cloned()).collect()))
    }
    pub fn get_metadata(&self, f: &BidsFile) -> Result<Option<PerfMetadata>> { Ok(self.layout.get_metadata(&f.path)?.deserialize_as()) }
    ...
    pub fn get_perf_subjects(&self) -> Result<Vec<String>> { self.layout.get().suffix("asl").return_unique("subject") }
    pub fn summary(&self) -> Result<PerfSummary> {
        ...
        Ok(PerfSummary { n_subjects: subjects.len(), n_scans: files.len(), subjects, asl_type })
    }
}

#[derive(Debug)]
pub struct PerfSummary { pub n_subjects: usize, pub n_scans: usize, pub subjects: Vec<String>, pub asl_type: Option<String> }
impl std::fmt::Display for PerfSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "Perfusion Summary: {} subjects, {} scans", self.n_subjects, self.n_scans)?;
        if let Some(ref t) = self.asl_type { writeln!(f, "  ASL Type: {t}")?; }
        Ok(())
    }
}
