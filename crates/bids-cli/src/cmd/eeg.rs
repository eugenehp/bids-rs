...
pub fn run(root: &PathBuf, subcmd: EegCommands) -> bids_core::error::Result<()> {
    ...
    match subcmd {
        EegCommands::Summary => { println!("{}", eeg.summary()?); }
        EegCommands::Channels { subject, task } => {
            ...
            for f in &query.collect()? {
                ...
                if let Some(channels) = eeg.get_channels(f)? {
                    println!("  {:>4}  {:>6}  {:>8}  {:>6}", "Name", "Type", "Units", "SFreq");
                    for ch in &channels {
                        println!("  {:>4}  {:>6}  {:>8}  {:>6}", ch.name,
                            ch.channel_type.to_string(), ch.units,
                            ch.sampling_frequency.map_or("n/a".into(), |f| f.to_string()));
                    }
                    println!("  Total: {} channels", channels.len());
                } else { println!("  No channels file found."); }
                println!();
            }
        }
        EegCommands::Events { subject, task } => {
            ...
            for f in &query.collect()? {
                ...
                if let Some(events) = eeg.get_events(f)? {
                    println!("  {:>8}  {:>8}  {:>15}  {:>8}", "Onset", "Duration", "Trial Type", "Value");
                    for ev in &events {
                        println!("  {:>8.3}  {:>8.3}  {:>15}  {:>8}", ev.onset, ev.duration,
                            ev.trial_type.as_deref().unwrap_or("n/a"),
                            ev.value.as_deref().unwrap_or("n/a"));
                    }
                    println!("  Total: {} events", events.len());
                } else { println!("  No events file found."); }
                println!();
            }
        }
    }
