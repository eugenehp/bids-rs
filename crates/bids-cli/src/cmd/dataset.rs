...
use bids_dataset::{Aggregator, Cache, DatasetFilter, OpenNeuro, Split};
use clap::Subcommand;
...
fn cmd_search(
    ...
    if let Some(ref m) = modality {
        search = search.modality(m);
    }
    if let Some(ref s) = species {
        search = search.species(s);
    }
    if let Some(ref k) = keyword {
        search = search.keyword(k);
    }
    let hits = search.execute()?;

    println!(
        "{:<12} {:<50} {:>10} {:>12}",
        "ID", "Name", "Modalities", "Size"
    );
    println!("{}", "─".repeat(86));
    for ds in &hits {
...
fn cmd_download(
    ...
    if force {
        let _ = cache.evict(&dataset_id, &ver);
    }
    ...
    if let Some(ref dir) = local_dir {
        let report = on.download_dataset(
            &dataset_id,
            dir,
            Some(move |f: &bids_dataset::RemoteFile| {
...
fn cmd_ls(
    ...
    let filtered: Vec<_> = if let Some(ref pat) = include {
        files
            .iter()
            .filter(|f| glob_matches(pat, &f.path))
            .collect()
    } else {
        files.iter().collect()
    };
                ...
                for ds in &datasets {
                    ...
                    if verbose {
                        for ver in &ds.versions {
                            ...
                            let complete = if cache.is_cached(&ds.id, ver) {
                                "✓"
                            } else {
                                "…"
                            };
...
fn cmd_aggregate(
    ...
    for ds_id in &datasets {
        let root = cache.resolve(ds_id, None).ok_or_else(|| {
            bids_core::BidsError::Io(std::io::Error::new(
        ...
        if let Some(ref m) = modality {
            filter = filter.modality(m);
        }
        if let Some(ref e) = extension {
            filter = filter.extension(e);
        }
...
fn cmd_env() -> bids_core::error::Result<()> {
    ...
    println!(
        "  BIDS_429_COOLDOWN_SECS: {}",
        cfg.default_429_cooldown_secs
    );
