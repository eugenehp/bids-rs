...
pub fn run(
    ...
    if let Some(ref s) = suffix {
        query = query.suffix(s);
    }
    if let Some(ref d) = datatype {
        query = query.datatype(d);
    }
    if let Some(ref e) = extension {
        query = query.extension(e);
    }
    ...
    for f in &files {
        println!(
            ...
            f.relpath(layout.root())
                .map_or_else(|| f.path.display().to_string(), |r| r.display().to_string())
        );
    }
