...
use crate::data::{Annotation, EegData, ReadOptions, read_eeg_data, read_brainvision_markers};
...
impl<'a> EegLayout<'a> {
    ...
    pub fn get_coordsystem(&self, f: &BidsFile) -> Result<Option<CoordinateSystem>> {
        ...
        if p.exists() { return Ok(Some(CoordinateSystem::from_file(&p)?)); }
        // Relaxed: try without task entity
        let stem = f.filename.split('.').next().unwrap_or("");
        let base_parts: Vec<&str> = stem.split('_')
            .filter(|p| p.starts_with("sub-") || p.starts_with("ses-") || p.starts_with("acq-"))
            .collect();
        if !base_parts.is_empty() {
            let relaxed = f.dirname.join(format!("{}_coordsystem.json", base_parts.join("_")));
            if relaxed.exists() { return Ok(Some(CoordinateSystem::from_file(&relaxed)?)); }
        }
        Ok(None)
    }
    ...
    pub fn get_physio(&self, f: &BidsFile) -> Result<Option<PhysioData>> {
        ...
        let sampling_freq = md.as_ref().and_then(|m| m.get_f64("SamplingFrequency")).unwrap_or(1.0);
        let start_time = md.as_ref().and_then(|m| m.get_f64("StartTime")).unwrap_or(0.0);
        let columns: Vec<String> = md.as_ref()
            .and_then(|m| m.get_array("Columns"))
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();
        Ok(Some(PhysioData { rows, sampling_frequency: sampling_freq, start_time, columns }))
    }

    pub fn read_edf_header(&self, f: &BidsFile) -> Result<Option<EdfHeader>> {
        if !f.filename.ends_with(".edf") && !f.filename.ends_with(".bdf") { return Ok(None); }
    ...
    pub fn get_all_channels_files(&self) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("channels").datatype("eeg").extension("tsv").collect()
    }
    pub fn get_all_electrodes_files(&self) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("electrodes").datatype("eeg").extension("tsv").collect()
    }
    pub fn get_all_events_files(&self) -> Result<Vec<BidsFile>> {
        self.layout.get().suffix("events").datatype("eeg").extension("tsv").collect()
    }
    ...
    pub fn summary(&self) -> Result<EegDatasetSummary> {
        ...
        let sampling_frequency = eeg_files.first()
            .and_then(|f| self.get_eeg_metadata(f).ok().flatten())
            .map(|m| m.sampling_frequency);
        let channel_count = eeg_files.first()
            .and_then(|f| self.get_channels(f).ok().flatten())
            .map(|c| c.len());
        Ok(EegDatasetSummary {
            n_subjects: subjects.len(), n_recordings: eeg_files.len(),
            subjects, tasks, sampling_frequency, channel_count,
        })
    }
}
...
impl EdfHeader {
    pub fn from_file(path: &std::path::Path) -> Result<Self> {
        ...
        let n_channels: usize = String::from_utf8_lossy(&header[252..256]).trim().parse().unwrap_or(0);
        let n_records: i64 = String::from_utf8_lossy(&header[236..244]).trim().parse().unwrap_or(-1);
        let record_duration: f64 = String::from_utf8_lossy(&header[244..252]).trim().parse().unwrap_or(1.0);
        ...
        for i in 0..n_channels {
            ...
            if offset + 16 <= ext_header.len() {
                channel_labels.push(String::from_utf8_lossy(&ext_header[offset..offset + 16]).trim().to_string());
            }
        }
        ...
        for i in 0..n_channels {
            ...
            if offset + 8 <= ext_header.len() {
                let samples: f64 = String::from_utf8_lossy(&ext_header[offset..offset + 8]).trim().parse().unwrap_or(0.0);
                if record_duration > 0.0 { sampling_rates.push(samples / record_duration); }
            }
        }
        Ok(Self { version, patient_id, recording_id, n_channels, n_records, record_duration, sampling_rates, channel_labels })
    }
    pub fn duration(&self) -> f64 {
        if self.n_records > 0 { self.n_records as f64 * self.record_duration } else { 0.0 }
    }
}
...
mod tests {
    ...
    fn test_companion() {
        ...
        assert_eq!(bf.companion("channels", "tsv").to_string_lossy(),
            "/data/sub-01/eeg/sub-01_task-rest_channels.tsv");
        assert_eq!(bf.companion("events", "tsv").to_string_lossy(),
