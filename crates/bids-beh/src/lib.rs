...
impl<'a> BehLayout<'a> {
    ...
    pub fn get_beh_files(&self) -> Result<Vec<BidsFile>> {
        self.layout.get().datatype("beh").collect()
    }
    pub fn get_beh_files_for_subject(&self, s: &str) -> Result<Vec<BidsFile>> {
        self.layout.get().datatype("beh").subject(s).collect()
    }
    pub fn get_beh_files_for_task(&self, t: &str) -> Result<Vec<BidsFile>> {
        self.layout.get().datatype("beh").task(t).collect()
    }
    pub fn get_beh_subjects(&self) -> Result<Vec<String>> {
        self.layout.get().datatype("beh").return_unique("subject")
    }
    pub fn get_beh_tasks(&self) -> Result<Vec<String>> {
        self.layout.get().datatype("beh").return_unique("task")
    }
...
impl std::fmt::Display for BehSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(
            f,
            "Behavioral Summary: {} subjects, {} files, tasks: {:?}",
            self.n_subjects, self.n_files, self.tasks
        )
    }
}
