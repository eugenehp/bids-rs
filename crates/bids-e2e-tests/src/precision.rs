...
fn precision_data() -> Value {
    ...
    let data = std::fs::read_to_string(&path).expect("Run `python tests/precision_test.py` first");
...
fn test_butter_all_orders_cutoffs() {
    ...
    for order in [1, 2, 3, 4, 5] {
        for cutoff in [0.1, 0.2, 0.3, 0.5, 0.8] {
            ...
            assert!(
                err_b < 1e-10,
                "{}: b max error {} (expected <1e-10)",
                key,
                err_b
            );
            assert!(
                err_a < 1e-10,
                "{}: a max error {} (expected <1e-10)",
                key,
                err_a
            );
        }
    ...
    println!(
        "Butterworth coefficients: {} configs tested, max error: {:.2e}",
        n_tests, max_err
    );
}
...
fn test_filtfilt_rms() {
    ...
    assert!(
        (rms - 0.707).abs() < 0.05,
        "filtfilt RMS should be ~0.707 (3Hz component), got {}",
        rms
    );
}
...
fn test_gamma_pdf_values() {
    ...
    for shape in [3.0f64, 6.0, 10.0] {
        for loc in [0.0f64, 0.04] {
            let shape_s = if shape.fract() == 0.0 {
                format!("{:.1}", shape)
            } else {
                format!("{}", shape)
            };
            let loc_s = if loc == 0.0 {
                "0.0".to_string()
            } else {
                format!("{}", loc)
            };
    ...
    println!(
        "Gamma PDF: {} points tested, max relative error: {:.2e}",
        n_points, max_err
    );
}
...
fn gammaln(x: f64) -> f64 {
    ...
    const S: [f64; 6] = [
        ...
        -2.77777777777777777778e-3,
        7.93650793650793650794e-4,
        -5.95238095238095238095e-4,
        8.41750841750841750842e-4,
        -1.91752691752691752692e-3,
    ];
    let mut s = S[5];
    for i in (0..5).rev() {
        s = s * inv_x2 + S[i];
...
fn test_hrf_values_all_configs() {
    ...
    for tr in [0.5f64, 1.0, 2.0] {
        for ov in [16usize, 50] {
            ...
            if let Some(expected) = g.get(&key) {
                ...
                assert_eq!(
                    hrf.len(),
                    expected["len"].as_u64().unwrap() as usize,
                    "{}: length mismatch",
                    key
                );
                ...
                let actual_peak = hrf
                    ...
                    .0;
                assert!(
                    (actual_peak as i64 - exp_peak as i64).unsigned_abs() <= 5,
                    "{}: peak {}, expected {} (±5)",
                    key,
                    actual_peak,
                    exp_peak
    ...
    println!(
        "HRF: {} configs tested, max RMS error: {:.2e}",
        n_configs, max_err
    );
}
