...
pub enum Instruction {
    Rename { input: Vec<String>, output: Vec<String> },
    Copy { input: Vec<String>, output: Vec<String> },
    Scale { input: Vec<String>, #[serde(default)] demean: bool, #[serde(default)] rescale: bool, #[serde(default)] replace_na: Option<f64> },
    Threshold { input: Vec<String>, #[serde(default = "default_threshold")] threshold: f64, #[serde(default)] above: bool, #[serde(default)] binarize: bool, #[serde(default)] signed: bool },
    And { input: Vec<String>, output: Option<Vec<String>> },
    Or { input: Vec<String>, output: Option<Vec<String>> },
    Not { input: Vec<String>, output: Option<Vec<String>> },
    Product { input: Vec<String>, output: Option<String> },
    Sum { input: Vec<String>, #[serde(default)] weights: Vec<f64>, output: Option<String> },
    Power { input: Vec<String>, value: f64, output: Option<Vec<String>> },
    Factor { input: Vec<String> },
    Filter { input: Vec<String>, query: String },
    Replace { input: Vec<String>, replace: HashMap<String, String>, output: Option<Vec<String>> },
    Select { input: Vec<String> },
    Delete { input: Vec<String> },
    Group { input: Vec<String>, output: String },
    Resample { input: Vec<String>, sampling_rate: f64 },
    ToDense { input: Vec<String>, sampling_rate: Option<f64> },
    Convolve { input: Vec<String>, #[serde(default = "default_hrf_model")] model: String },
}

fn default_threshold() -> f64 { 0.0 }
fn default_hrf_model() -> String { "spm".into() }
...
fn get_inputs(instruction: &serde_json::Value) -> Vec<String> {
    instruction.get("Input")
        ...
        .or_else(|| instruction.get("Input").and_then(|v| v.as_str()).map(|s| vec![s.into()]))
        .unwrap_or_default()
}
...
fn get_outputs(instruction: &serde_json::Value) -> Vec<String> {
    instruction.get("Output")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
        .or_else(|| instruction.get("Output").and_then(|v| v.as_str()).map(|s| vec![s.into()]))
        .unwrap_or_default()
}
...
fn apply_factor(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    ...
    for input_name in &inputs {
        if let Some(var) = collection.variables.get(input_name) {
            ...
            let unique: Vec<String> = str_values.iter()
                .filter(|v| !v.is_empty() && seen.insert((*v).clone()))
                .cloned().collect();

            for level in &unique {
                let new_name = format!("{input_name}.{level}");
                let values: Vec<String> = str_values.iter()
                    .map(|v| if v == level { "1".into() } else { "0".into() })
                    .collect();
                new_vars.push(SimpleVariable::new(&new_name, &source, values, index.clone()));
            }
        }
    }
...
fn apply_replace(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let outputs = get_outputs(instruction);
    let replace_map: HashMap<String, String> = instruction.get("Replace")
        .and_then(|v| serde_json::from_value(v.clone()).ok())
        .unwrap_or_default();

    for (i, input_name) in inputs.iter().enumerate() {
        if let Some(var) = collection.variables.get(input_name) {
            let new_values: Vec<String> = var.str_values.iter()
...
fn apply_scale(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let demean = instruction.get("Demean").and_then(serde_json::Value::as_bool).unwrap_or(false);
    let rescale = instruction.get("Rescale").and_then(serde_json::Value::as_bool).unwrap_or(false);

    for input_name in &inputs {
        if let Some(var) = collection.variables.get_mut(input_name) {
            if !var.is_numeric { continue; }
            let vals = &var.values;
            let finite: Vec<f64> = vals.iter().copied().filter(|v| v.is_finite()).collect();
            if finite.is_empty() { continue; }

            let mean = finite.iter().sum::<f64>() / finite.len() as f64;
            let std = (finite.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / finite.len() as f64).sqrt();

            for (i, v) in var.values.iter_mut().enumerate() {
                if !v.is_finite() { continue; }
                if demean { *v -= mean; }
                if rescale && std > 1e-15 { *v /= std; }
...
fn apply_threshold(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let threshold = instruction.get("Threshold").and_then(serde_json::Value::as_f64).unwrap_or(0.0);
    let above = instruction.get("Above").and_then(serde_json::Value::as_bool).unwrap_or(true);
    let binarize = instruction.get("Binarize").and_then(serde_json::Value::as_bool).unwrap_or(false);

    for input_name in &inputs {
        if let Some(var) = collection.variables.get_mut(input_name) {
            if !var.is_numeric { continue; }
            for (i, v) in var.values.iter_mut().enumerate() {
                let passes = if above { *v >= threshold } else { *v <= threshold };
                if binarize {
...
fn apply_dropna(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    ...
    for input_name in &inputs {
        if let Some(var) = collection.variables.get(input_name) {
            let keep: Vec<usize> = var.str_values.iter().enumerate()
                .filter(|(_, v)| !v.is_empty())
                .map(|(i, _)| i)
                .collect();
            let new_values: Vec<String> = keep.iter().map(|&i| var.str_values[i].clone()).collect();
            let new_index: Vec<StringEntities> = keep.iter().filter_map(|&i| var.index.get(i).cloned()).collect();
        ...
        if let Some(var) = collection.variables.get(input_name) {
            ...
            if let Some(group_var) = by_var {
                ...
                for (key, indices) in &groups {
                    ...
                    let values: Vec<String> = indices.iter().map(|&i| var.str_values.get(i).cloned().unwrap_or_default()).collect();
                    let index: Vec<StringEntities> = indices.iter().filter_map(|&i| var.index.get(i).cloned()).collect();
                    new_vars.push(SimpleVariable::new(&name, &var.source, values, index));
                }
            }
        }
    }
    for v in new_vars { collection.variables.insert(v.name.clone(), v); }
}

fn apply_concatenate(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let output = instruction.get("Output").and_then(|v| v.as_str())
...
fn apply_orthogonalize(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let other_names: Vec<String> = instruction.get("Other")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
        .unwrap_or_default();
        ...
        if let Some(var) = collection.variables.get(input_name) {
            ...
            for other_name in &other_names {
                if let Some(other) = collection.variables.get(other_name) {
                    if other.values.len() != x.len() { continue; }
            ...
            let new_var = SimpleVariable::new(&var.name, &var.source, new_values, var.index.clone());
            collection.variables.insert(input_name.clone(), new_var);
        }
    }
}
...
fn apply_lag(collection: &mut VariableCollection, instruction: &serde_json::Value) {
    let inputs = get_inputs(instruction);
    let n_shift = instruction.get("N").and_then(serde_json::Value::as_i64).unwrap_or(1);
    let outputs = get_outputs(instruction);
    ...
    for (i, input_name) in inputs.iter().enumerate() {
        if let Some(var) = collection.variables.get(input_name) {
            ...
            let lagged: Vec<f64> = (0..n).map(|j| {
                let src = j as i64 - n_shift;
                if src >= 0 && (src as usize) < n { var.values[src as usize] } else { 0.0 }
            }).collect();
            let new_values: Vec<String> = lagged.iter().map(std::string::ToString::to_string).collect();
            let out_name = outputs.get(i).unwrap_or(input_name);
            let new_var = SimpleVariable::new(out_name, &var.source, new_values, var.index.clone());
            collection.variables.insert(out_name.clone(), new_var);
        }
...
impl TransformerManager {
    pub fn new(transformer: &str, keep_history: bool) -> Self {
        Self { transformer: transformer.into(), keep_history, history: Vec::new() }
    }

    pub fn transform(&mut self, mut collection: VariableCollection, spec: &TransformSpec) -> VariableCollection {
        for instruction in &spec.instructions {
...
pub fn expand_wildcards(selectors: &[String], pool: &[String]) -> Vec<String> {
    ...
    for spec in selectors {
        if spec.contains('*') || spec.contains('?') || spec.contains('[') {
            let re_str = format!("^{}$",
                spec.replace('.', r"\.").replace('*', ".*").replace('?', "."));
            if let Ok(re) = regex::Regex::new(&re_str) {
                for name in pool {
                    if re.is_match(name) { out.push(name.clone()); }
                }
            }
        } else {
            out.push(spec.clone());
...
mod tests {
    ...
    fn make_collection() -> VariableCollection {
        use bids_core::entities::StringEntities;
        let v1 = SimpleVariable::new("trial_type", "events",
            vec!["face".into(), "house".into(), "face".into()],
            vec![StringEntities::new(); 3]);
        let v2 = SimpleVariable::new("rt", "events",
    ...
    fn test_factor() {
        ...
        assert_eq!(col.variables["trial_type.face"].str_values, vec!["1", "0", "1"]);
    }

    #[test]
    fn test_rename() {
        let mut col = make_collection();
        let instr = serde_json::json!({"Name": "Rename", "Input": ["rt"], "Output": ["reaction_time"]});
    ...
    fn test_scale() {
        let mut col = make_collection();
        let instr = serde_json::json!({"Name": "Scale", "Input": ["rt"], "Demean": true, "Rescale": true});
        apply_scale(&mut col, &instr);
        let vals = &col.variables["rt"].values;
        let mean: f64 = vals.iter().sum::<f64>() / vals.len() as f64;
        assert!(mean.abs() < 1e-10, "Mean should be ~0 after demean, got {}", mean);
    }
